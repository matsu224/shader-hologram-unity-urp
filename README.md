...
ハッカソンで配布された3DCGモデルやコードは公開できないため、自作した`.shader`ファイルとREADMEのみ公開します。
...
色や透明度、スキャンラインの速度・幅、Fresnel効果、明滅、グリッチなどの強さは、マテリアルから調整可能。
...
6. 速度や幅が異なるスキャンラインを重ね、帯の幅と比率も調整可能にすることで、さらなるホログラム感を演出
...
8. 全体の明滅を実装
   ...
   → おまけとしてシンプルに実装したかったため、`sin`と`_Time`を用いて周期的に小さな値を乗算<br>
   → 想像以上に見た目の良い結果となった
